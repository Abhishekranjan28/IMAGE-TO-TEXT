...
function AboutUs() {
    ...
    return (
        <div className='pt-[5vw] flex justify-center items-center flex-col overflow-hidden min-h-[100vh]'>
            <div className='flex  justify-evenly w-full items-center'>
                <div className='text-white text-[4vw] font-bold '>ABOUT US</div>
                <div className='text-red-300 text-[2vw] hover:underline'>
                    <Link to={'/upload'}>UPLOAD AND TRY</Link>
                </div>
            </div>
            {aboutUsData.map((section, index) => (
                <div key={index} className='section bg-[rgba(10,31,60,1)]/80 shadow-2xl hover:shadow-white  w-[85vw] cursor-pointer border-2 border-blue-400 p-4 mt-10 hover:scale-105 rounded-lg hover:shadow-lg transition-transform duration-300 ease-in-out'>
                    ...
                    >
                        <div className='w-1/3 bg-cover bg-no-repeat bg-center overflow-hidden rounded-3xl transition-transform duration-300 ease-in-out min-h-[100px]' style={{ backgroundImage: `url(${section.image})` }}></div>
